...
public class GhnApiClient {
    ...
    public GhnCreateOrderResult createOrder(GhnCreateOrderRequest request) {
        ...
        logger.debug(
            "GHN create-order response: orderCode={}, trackingNumber={}, sortCode={}, totalFee={}, expectedDeliveryTime={}",
            orderCode,
            trackingNumber,
            sortCode,
            totalFee,
    ...
    /**
     * Ask GHN to switch existing orders into return flow.
     * API: /v2/switch-status/return
     */
    public void switchStatusToReturn(long shopId, List<String> orderCodes) {
        ensureEnabled();
        if (orderCodes == null || orderCodes.isEmpty()) {
            throw new GhnApiException("GHN switch-return requires at least one order code");
        }
        Map<String, Object> payload = new HashMap<>();
        if (shopId > 0) {
            payload.put("shop_id", shopId);
        }
        payload.put("order_codes", orderCodes);
        logger.info("GHN switch-return request: shopId={}, orderCodes={}", shopId, orderCodes);
        JsonNode data = post(config.getSwitchReturnPath(), payload);
        validateSwitchReturnResult(data);
    }
    ...
    private JsonNode post(String path, Map<String, Object> payload) {
        ...
        try {
            ...
            logger.info("GHN API response: path={}, httpStatus={}, body={}",
    ...
    private void validateSwitchReturnResult(JsonNode data) {
        if (data == null || !data.isArray() || data.isEmpty()) {
            throw new GhnApiException("GHN switch-return response missing result list");
        }
        JsonNode first = data.get(0);
        boolean ok = first.path("result").asBoolean(false);
        if (!ok) {
            String orderCode = first.path("order_code").asText(null);
            String message = first.path("message").asText("switch return failed");
            throw new GhnApiException("GHN switch-return failed for orderCode="
                + orderCode + ", message=" + message);
        }
        String message = first.path("message").asText("");
        if (!"OK".equalsIgnoreCase(message) && !"SUCCESS".equalsIgnoreCase(message)
            && !message.toLowerCase(Locale.ROOT).contains("success")) {
            logger.warn("GHN switch-return non-standard success message: {}", message);
        }
    }
