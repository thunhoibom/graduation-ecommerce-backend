...
public interface OrdersRepository
    ...
    @Query(value = "SELECT DISTINCT s FROM Order s "
        + "JOIN FETCH s.details "
        + "WHERE s.transactionToken = :token")
    Optional<Order> findByTransactionTokenWithDetails(@Param("token") String token);
    ...
    @Query(value = """
        SELECT DATE(o.order_date)               AS periodDate,
               COALESCE(SUM(GREATEST(o.order_total_value - o.order_total_refunded_amount, 0)), 0) AS revenue,
               COUNT(*) FILTER (
                   WHERE GREATEST(o.order_total_value - o.order_total_refunded_amount, 0) > 0
               ) AS orderCount
        FROM   orders o
    ...
    @Query("SELECT o FROM Order o "
        ...
        + "WHERE p.id = :personId "
        + "ORDER BY o.date DESC")
    List<Order> findByCustomerPersonId(@Param("personId") Long personId);
    ...
    @Query(value = "SELECT * FROM orders o ORDER BY o.order_date DESC LIMIT :limit", nativeQuery = true)
