...
public interface UsersRepository
    ...
    @Query("SELECT u FROM User u JOIN FETCH u.person p WHERE p.id = :personId ORDER BY u.id ASC")
    List<User> findAllByPersonIdOrderByIdAsc(@Param("personId") Long personId);

    boolean existsByPerson_Id(Long personId);

    default Optional<User> findFirstByPersonId(Long personId) {
        List<User> users = findAllByPersonIdOrderByIdAsc(personId);
