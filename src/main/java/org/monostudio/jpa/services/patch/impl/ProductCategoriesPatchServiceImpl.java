...
public class ProductCategoriesPatchServiceImpl
    ...
    public ProductCategory patchExistingEntity(Map<String, Object> changes, ProductCategory existing) throws BadInputException {
        ...
        if (changes.containsKey("displayOrder")) {
            Object orderValue = changes.get("displayOrder");
            if (orderValue == null) {
                target.setDisplayOrder(0);
            } else if (orderValue instanceof Number number) {
                target.setDisplayOrder(Math.max(0, number.intValue()));
            }
        }
