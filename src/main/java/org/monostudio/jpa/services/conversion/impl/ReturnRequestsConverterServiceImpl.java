...
public class ReturnRequestsConverterServiceImpl
    ...
    public ReturnRequestPojo convertToPojo(@NotNull ReturnRequest source) {
        ReturnRequestPojo.ReturnRequestPojoBuilder builder = ReturnRequestPojo.builder()
            ...
            .refundBankName(source.getRefundBankName())
            .refundBankAccountNumber(source.getRefundBankAccountNumber())
            .refundBankAccountHolder(source.getRefundBankAccountHolder())
            .refundProofUrl(source.getRefundProofUrl())
            .refundReference(source.getRefundReference())
            .refundedAt(source.getRefundedAt())
            .qcStatus(source.getQcStatus() != null ? source.getQcStatus().name() : null)
            .qcNotes(source.getQcNotes())
            .qcPhotoUrls(source.getQcPhotoUrls())
            .qcCompletedAt(source.getQcCompletedAt());
        ...
        if (source.getOrder() != null) {
            ...
            applyOrderRecipient(builder, source.getOrder());
        }

        return builder.build();
    }

    private static void applyOrderRecipient(ReturnRequestPojo.ReturnRequestPojoBuilder builder, Order order) {
        if (order == null || order.getCustomer() == null || order.getCustomer().getPerson() == null) {
            return;
        }
        Person person = order.getCustomer().getPerson();
        String fullName = String.format("%s %s", person.getFirstName(), person.getLastName()).trim();
        if (!fullName.isBlank()) {
            builder.orderRecipientName(fullName);
        }
        if (person.getPhone1() != null && !person.getPhone1().isBlank()) {
            builder.orderRecipientPhone(person.getPhone1());
        }
    }
