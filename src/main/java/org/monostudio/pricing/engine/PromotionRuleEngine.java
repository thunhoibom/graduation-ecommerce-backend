...
public class PromotionRuleEngine {
    ...
    public List<PromotionRule> findMatchingRules(List<PromotionRule> activeRules, PricingFacts facts) {
        ...
        for (PromotionRule rule : activeRules) {
            if (ruleConditionsMatch(rule, facts)) {
                out.add(rule);
            }
        }
        return out;
    ...
    /**
     * True iff every condition on the rule passes for the given facts.
     */
    public boolean ruleConditionsMatch(PromotionRule rule, PricingFacts facts) {
        if (rule.getConditions() == null || rule.getConditions().isEmpty()) {
            return true;
        }
        for (PromotionRuleCondition c : rule.getConditions()) {
            if (!singleConditionMatches(c, facts)) {
                return false;
            }
        }
        return true;
    ...
    /**
     * Evaluate a single condition (same semantics as {@link #ruleConditionsMatch}).
     */
    public boolean singleConditionMatches(PromotionRuleCondition c, PricingFacts facts) {
        if (isCategoryPresenceCondition(c)) {
            if (!"IN".equalsIgnoreCase(c.getOperator() == null ? "" : c.getOperator().trim())) {
                return false;
            }
            return conditionEvaluator.categoryAnyMatch(c.getTargetValue(), facts);
        }
        if (isProductPresenceCondition(c)) {
            if (!"IN".equalsIgnoreCase(c.getOperator() == null ? "" : c.getOperator().trim())) {
                return false;
            }
            return conditionEvaluator.productAnyMatch(c.getTargetValue(), facts);
        }
        return conditionEvaluator.ruleMatches(c, facts);
    }
