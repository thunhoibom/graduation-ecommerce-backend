...
public class StockCountServiceImpl
    ...
    public List<StockCountSessionPojo> list(String status) {
        ...
        return stockCountSessionsRepository.findByStatus(parsed).stream()
            .map(session -> toPojoWithLines(session))
    ...
    public StockCountSessionPojo getById(Long id) {
        return toPojoWithLines(getSessionOrThrow(id));
    }
    ...
    public StockCountSessionPojo start(Long id, Long actorId, String note) {
        ...
        replaceSessionLines(session, lines);
        ...
        stockCountSessionsRepository.save(session);
        return toPojoWithLines(session);
    }
    ...
    private StockCountSessionPojo toPojoWithLines(StockCountSession session) {
        replaceSessionLines(session, stockCountLinesRepository.findByStockCountSessionId(session.getId()));
        return InventoryMapper.toPojo(session);
    }

    private void replaceSessionLines(StockCountSession session, List<StockCountLine> lines) {
        List<StockCountLine> target = session.getLines();
        if (target == null) {
            session.setLines(new ArrayList<>(lines));
            return;
        }
        target.clear();
        target.addAll(lines);
