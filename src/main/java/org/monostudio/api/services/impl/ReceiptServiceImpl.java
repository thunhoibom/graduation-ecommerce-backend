...
public class ReceiptServiceImpl
    ...
    public ReceiptPojo fetchReceiptByTransactionToken(String token)
        ...
        Optional<Order> match = ordersRepository.findByTransactionTokenWithDetails(token);
        if (match.isEmpty()) {
            match = findByNumericOrderId(token);
        }
        if (match.isEmpty()) {
    ...
    private Optional<Order> findByNumericOrderId(String token) {
        try {
            return ordersRepository.findByIdWithDetails(Long.parseLong(token));
