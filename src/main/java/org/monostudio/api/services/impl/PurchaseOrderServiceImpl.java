...
public class PurchaseOrderServiceImpl
    ...
    public List<PurchaseOrderPojo> list(String status) {
        ...
        List<PurchaseOrder> orders = purchaseOrdersRepository.findAllByStatusDeep(parsedStatus);
        if (orders.isEmpty()) {
            return List.of();
        }
        List<Long> purchaseOrderIds = orders.stream().map(PurchaseOrder::getId).toList();
        Map<Long, InventoryMapper.PurchaseOrderTotals> totalsById = purchaseOrderLinesRepository
            .summarizeByPurchaseOrderIds(purchaseOrderIds)
            .stream()
            .collect(Collectors.toMap(
                PurchaseOrderLineTotalsProjection::getPurchaseOrderId,
                InventoryMapper::toTotals
            ));
        return orders.stream()
            .map(order -> {
                PurchaseOrderPojo pojo = InventoryMapper.toPojo(order);
                return InventoryMapper.applyTotals(
                    pojo,
                    totalsById.getOrDefault(order.getId(), InventoryMapper.PurchaseOrderTotals.empty())
                );
            })
