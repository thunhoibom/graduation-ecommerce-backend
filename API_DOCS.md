...
### `POST /public/cart/calculate` — Calculate Cart Pricing (promotion rules + coupon)

**Security:** permitAll

**Headers:** `X-Session-Token` (required — invalid/missing token returns error)
...
Chạy engine khuyến mãi CART/SHIPPING, gộp coupon (nếu có), và lưu snapshot lên session giỏ.

**Response — CartPricingResult**

| Field | Description |
|-------|-------------|
| `promotionNearMisses` | Gợi ý khi giỏ **gần** đạt ngưỡng `cart.subtotal` (chỉ rule chưa khớp mà mọi điều kiện khác đã đúng và chỉ thiếu GTE/GT trên subtotal). |

```json
{
  "subtotal": 400000,
  "discountAmount": 0,
  "totalAfterDiscount": 400000,
  "freeShipping": false,
  "appliedPromotions": [],
  "promotionNearMisses": [
    {
      ...
      "messageVi": "Thêm 100.000 ₫ nữa để đủ điều kiện \"Freeship đơn 500k\".",
...
### `GET /public/about` — Thông tin doanh nghiệp (storefront)

**Security:** permitAll

Trả về `CompanyDetailsPojo` (tên, mô tả, logo, banner, địa chỉ liên hệ công khai, …) — dùng cho trang Giới thiệu / đồng bộ thông tin cửa hàng.

---

### `POST /public/contact` — Gửi form liên hệ (storefront)

**Security:** permitAll

**Rate limit:** tối đa **5** request/phút cho mỗi địa chỉ IP. Vượt quá trả **429** với JSON `{ "code": "RATE_CONTACT", "message": "...", "canRetry": true }`.

**Body (JSON):**

| Field | Bắt buộc | Ghi chú |
|-------|----------|---------|
| `name` | Có | Tối đa 200 ký tự |
| `email` | Có | Email hợp lệ |
| `phone` | Không | Chuỗi rỗng được; nếu có thì dạng `^[0-9+\\s]{8,15}$` |
| `subject` | Có | Một trong: `order`, `product`, `return`, `cooperation`, `feedback`, `other` |
| `message` | Có | 10–8000 ký tự |
...
```json
{
  ...
  "phone": "0901234567",
  "subject": "product",
  "message": "Toi muon hoi ve size ao..."
}
```
...
Tin được ghi vào bảng `contact_inquiries` và (best-effort) gửi email thông báo tới địa chỉ cấu hình `monostudio.mailing.contact-inquiry-notification-to`, hoặc `monostudio.mailing.owner-email` nếu để trống.

**Lỗi 400:** validation — `AppError` với `code` `REJECTED_02`, `detailMessage` liệt kê field.

---

### `GET /public/promotions` — Danh sách promotion đang hiệu lực (storefront)
...
| Param | Description |
|-------|-------------|
| `productBarcode` | (optional) Luôn trả các rule không giới hạn `cart.has_any_product`; rule có điều kiện sản phẩm chỉ giữ khi barcode nằm trong danh sách IN. |
...
[
  {
    ...
    "scope": "CART",
    "effectsSummary": "Giảm 10%",
    "conditionsSummary": "Đơn từ 500.000 ₫",
    "reflectedInProductPrice": null,
    "activeUntil": "2026-12-31T23:59:59"
  }
]
```

`scope` có thể là `PRODUCT`, `CATEGORY`, `CART`, `SHIPPING`. Khi `reflectedInProductPrice` = `true`, giá đã phản ánh trên PDP/catalog.

**QA gợi ý:** (1) Rule chỉ `cart.subtotal` `GTE` với giỏ dưới ngưỡng → `POST /calculate` trả `promotionNearMisses`. (2) Rule thêm điều kiện category/product không thỏa → không có near-miss. (3) `GET /promotions` không query vs có `productBarcode` khớp rule `cart.has_any_product`. (4) Áp dụng mã giảm → gọi `POST /calculate` với `couponCode`, sau đó client gọi lại calculate kèm mã đã lưu để không mất coupon.
...
### `GET /data/products?barcode=X` — Filter Products by Barcode

> Note: `barcode` is a **filter query parameter** on the standard list endpoint, not a separate endpoint.

```json
// GET /api/data/products?barcode=123456789
// Response 200 — DataPagePojo with matching ProductPojo
```
